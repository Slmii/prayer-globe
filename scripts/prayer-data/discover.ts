...
interface RegList {
	...
	StateRegionList: { IlceAdi: string; IlceAdiEn: string; IlceID: string; IlceUrl: string }[] | null;
}
...
async function statesOf(countryId: string): Promise<RegList['StateList']> {
	const res = await getJson<RegList>(`/en-US/home/GetRegList?ChangeType=country&CountryId=${countryId}`);
...
async function main() {
	...
	for (const [i, country] of countries.entries()) {
		...
		const built = await mapPool(states, 3, async s => ({
			...
			nameEn: s.SehirAdiEn,
			districts: (
				await districtsOf(country.countryId, s.SehirID)
			).map(d => ({
				ilceID: d.IlceID,
				name: d.IlceAdi,
				nameEn: d.IlceAdiEn,
	...
	const districts = tree.reduce((sum, c) => sum + c.states.reduce((s, st) => s + st.districts.length, 0), 0);
