 ...
 * another. `ilceID` pins the district outright, either because the two
 * sources' spellings are too far apart to match, or because the city would
 * otherwise lose the per-country population cut to bigger cities that matched.
 */
...
const REQUIRED_CITIES: RequiredCity[] = [
	...
	},
	{
		...
		geoIso2: 'CN',
		ilceID: '12368',
		why: "Great Mosque of Xi'an has a 3D model; China's six biggest matched cities outrank it, so it misses the population cut"
	},
	{
		name: "Samarra'",
		geoIso2: 'IQ',
		ilceID: '14169',
		why: "Great Mosque of Samarra has a 3D model; it misses the population cut against Iraq's bigger matched cities"
	}
];
...
async function main() {
	...
	const byIso2 = new Map<
		string,
		{ country: DiyanetTree[number]; districts: DiyanetDistrict[]; stateOf: Map<string, string> }
	>();
	...
	for (const req of REQUIRED_CITIES) {
		...
		if (!district) {
			throw new Error(`required city ${req.name} matched no district in ${hostIso2} — ${req.why}`);
		}
