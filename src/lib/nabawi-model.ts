...
/* crenellated parapet */
function parapet(M: Materials, len: number, name: string, pos: [number, number, number], rotY?: number) {
...
function smallDome(M: Materials, r: number, tag: string) {
	...
	g.add(drum);
	const shell = new THREE.Mesh(new THREE.SphereGeometry(r, 26, 14, 0, Math.PI * 2, 0, Math.PI / 2), M.white);
...
function minaret(M: Materials, tag: string) {
	...
	put(new THREE.BoxGeometry(8.0, 1.0, 8.0), M.stoneShade, 'minaret_base_cornice_' + tag, [0, 20.3, 0]);
	put(new THREE.CylinderGeometry(3.0, 3.9, 5.0, 8), M.stone, 'minaret_transition_' + tag, [0, 23.3, 0]);
	put(new THREE.CylinderGeometry(2.5, 3.0, 26, 8), M.stone, 'minaret_shaft_lower_' + tag, [0, 38.8, 0]);
	/* first balcony */
	put(new THREE.CylinderGeometry(4.0, 3.2, 1.2, 16), M.stoneShade, 'balcony1_floor_' + tag, [0, 52.4, 0]);
	put(new THREE.CylinderGeometry(3.9, 3.9, 2.6, 16, 1, true), M.green, 'balcony1_rail_' + tag, [0, 54.3, 0]);
	put(new THREE.CylinderGeometry(2.0, 2.4, 14, 8), M.stone, 'minaret_shaft_upper_' + tag, [0, 62.6, 0]);
	/* second balcony */
	put(new THREE.CylinderGeometry(3.2, 2.6, 1.0, 16), M.stoneShade, 'balcony2_floor_' + tag, [0, 70.1, 0]);
	put(new THREE.CylinderGeometry(3.1, 3.1, 2.2, 16, 1, true), M.green, 'balcony2_rail_' + tag, [0, 71.7, 0]);
	put(new THREE.CylinderGeometry(1.7, 1.9, 8, 8), M.stone, 'minaret_lantern_shaft_' + tag, [0, 76.8, 0]);
	/* green bulb cap */
	const cap = put(new THREE.SphereGeometry(2.5, 24, 16), M.green, 'minaret_cap_' + tag, [0, 82.6, 0]);
	cap.scale.set(1, 1.35, 1);
	put(new THREE.ConeGeometry(2.4, 3.2, 20), M.green, 'minaret_cap_cone_' + tag, [0, 86.6, 0]);
	put(new THREE.CylinderGeometry(0.22, 0.3, 2.6, 12), M.gold, 'minaret_finial_' + tag, [0, 89.4, 0]);
...
function umbrella(M: Materials, tag: string) {
	...
	put(new THREE.CylinderGeometry(0.6, 0.8, 13, 14), M.stoneShade, 'canopy_mast_' + tag, [0, 6.5, 0]);
	put(new THREE.BoxGeometry(15.5, 0.55, 15.5), M.white, 'canopy_shade_' + tag, [0, 13.4, 0]);
	for (let i = 0; i < 4; i++) {
		const r = put(new THREE.BoxGeometry(15.5, 0.3, 0.55), M.white, `canopy_rib_${tag}_${i}`, [0, 13.0, 0]);
...
export function buildNabawi(): THREE.Group {
	...
	const HALL = 24;
	put(new THREE.BoxGeometry(W - 10, WALL - 1.5, HALL), M.stoneWarm, 'prayer_hall_south', [
		0,
		Y + (WALL - 1.5) / 2,
		D / 2 - HALL / 2 - 4
	]);
	put(new THREE.BoxGeometry(W - 10, WALL - 1.5, HALL), M.stoneWarm, 'prayer_hall_north', [
		0,
		Y + (WALL - 1.5) / 2,
		-D / 2 + HALL / 2 + 4
	]);
	...
	/* cornice + roof deck */
	put(new THREE.BoxGeometry(W + 1.6, 1.1, D + 1.6), M.stoneShade, 'cornice', [0, Y + WALL - 0.55, 0]);
	...
	put(new THREE.BoxGeometry(W - 46, 0.4, D - 2 * HALL - 10), M.marble, 'sahn_floor', [0, Y + 0.2, 0]);

	/* ---------- the Green Dome over the Prophet's chamber ---------- */
	const gd = new THREE.Group();
	gd.name = 'green_dome';
	const gdR = 9.2;
	put(new THREE.BoxGeometry(gdR * 2.5, 6.5, gdR * 2.5), M.stone, 'green_dome_square_base', [0, 3.25, 0], gd);
	put(new THREE.BoxGeometry(gdR * 2.7, 0.7, gdR * 2.7), M.stoneShade, 'green_dome_base_cornice', [0, 6.6, 0], gd);
	put(new THREE.CylinderGeometry(gdR, gdR * 1.04, 7.4, 32), M.stone, 'green_dome_drum', [0, 10.6, 0], gd);
	for (let i = 0; i < 12; i++) {
	...
	put(new THREE.TorusGeometry(gdR + 0.25, 0.4, 12, 40), M.greenDeep, 'green_dome_ring', [0, 14.4, 0], gd).rotation.x =
		Math.PI / 2;
	...
	put(new THREE.SphereGeometry(0.85, 20, 14), M.gold, 'green_dome_finial_bulb', [0, 17.7 + gdR * 1.24, 0], gd);
	const cres = put(
	...
	put(new THREE.BoxGeometry(24, WALL - 1.5, 24), M.stoneWarm, 'chamber_mass', [0, (WALL - 1.5) / 2, 0], chamber);
	for (let i = 0; i < 6; i++) {
		put(
	...
	for (let i = 0; i < 5; i++) {
		put(new THREE.BoxGeometry(3.3, 2.4, 1.4), M.stone, 'portal_merlon_' + i, [-7 + i * 3.5, 23.2, 0], portal);
	}
