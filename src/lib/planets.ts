...
export const centuriesSinceJ2000 = (date: Date) => (date.getTime() / 86400000 + 2440587.5 - 2451545.0) / 36525;
...
export const gmstDeg = (date: Date) => 280.147 + 360.9856235 * (date.getTime() / 86400000 + 2440587.5 - 2451545.0);
...
export function planetStates(date: Date): PlanetState[] {
	...
	return PLANETS.map(el => {
		...
		const dec =
			Math.asin(
				Math.sin(eclipticLat * D) * Math.cos(OBLIQUITY) + Math.cos(eclipticLat * D) * Math.sin(OBLIQUITY) * sl
			) / D;
