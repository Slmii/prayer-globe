...
export function buildKaaba(): THREE.Group {
  ...
  put(new THREE.BoxGeometry(W + 0.34, 0.12, D + 0.34), M.marbleGrey, 'shadharwan_cap', [
    0, 0.78, 0,
  ])
  ...
  for (const [axis, len, count, sgn] of ribGroups) {
    for (let i = 0; i < count; i++) {
      ...
      const rib = new THREE.Mesh(
        new THREE.CylinderGeometry(0.075, 0.075, H - 0.3, 8),
        M.kiswah,
      )
  ...
  put(new THREE.BoxGeometry(W + 0.42, 0.07, D + 0.42), M.goldDeep, 'hizam_edge_top', [
    0, hzY + 0.5, 0,
  ])
  put(new THREE.BoxGeometry(W + 0.42, 0.07, D + 0.42), M.goldDeep, 'hizam_edge_bottom', [
    0, hzY - 0.5, 0,
  ])
  ...
  /* crown band and roof */
  put(new THREE.BoxGeometry(W + 0.5, 0.42, D + 0.5), M.goldDeep, 'crown_band', [
    0, y0 + H - 0.2, 0,
  ])
  put(new THREE.BoxGeometry(W + 0.2, 0.3, D + 0.2), M.marbleGrey, 'roof_slab', [
    0, y0 + H + 0.15, 0,
  ])
  put(new THREE.BoxGeometry(W + 0.6, 0.5, 0.34), M.marble, 'roof_parapet_south', [
    0, y0 + H + 0.5, D / 2 + 0.2,
  ])
  put(new THREE.BoxGeometry(W + 0.6, 0.5, 0.34), M.marble, 'roof_parapet_north', [
    0, y0 + H + 0.5, -D / 2 - 0.2,
  ])
  put(new THREE.BoxGeometry(0.34, 0.5, D + 0.6), M.marble, 'roof_parapet_east', [
    W / 2 + 0.2, y0 + H + 0.5, 0,
  ])
  put(new THREE.BoxGeometry(0.34, 0.5, D + 0.6), M.marble, 'roof_parapet_west', [
    -W / 2 - 0.2, y0 + H + 0.5, 0,
  ])
