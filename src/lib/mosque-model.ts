...
function domeProfile(R: number, lift: number, segs = 40) {
  ...
  for (let i = 0; i <= segs; i++) {
    ...
    pts.push(
      new THREE.Vector2(R * Math.cos(a) * (1 + 0.06 * Math.sin(a * 2)), R * Math.sin(a) * lift),
    )
  }
  return pts
}
...
function finial(M: Materials, scale: number, tag: string) {
  ...
  const stem = new THREE.Mesh(
    new THREE.CylinderGeometry(0.12 * scale, 0.16 * scale, 1.4 * scale, 16),
    M.brass,
  )
  ...
  const cres = new THREE.Mesh(
    new THREE.TorusGeometry(0.52 * scale, 0.075 * scale, 12, 40, Math.PI * 1.45),
    M.brass,
  )
...
export function buildMosque(): THREE.Group {
  ...
  add(
    new THREE.Mesh(new THREE.BoxGeometry(PL - 2.4, 0.35, PD - 2.4), M.limestone),
    'plinth_top',
  ).position.y = 0.9 + 0.175
  ...
  add(
    new THREE.Mesh(new THREE.BoxGeometry(HW + 0.9, 0.7, HD + 0.9), M.stoneDark),
    'cornice',
  ).position.y = BASE + HH - 0.35
  ...
  model.add(front)
  add(
    new THREE.Mesh(new THREE.BoxGeometry(HW - 0.2, 8.6, 0.4), M.void_),
    'front_recess',
  ).position.set(0, BASE + 4.3, HD / 2 - 0.1)
  ...
  const domeY = BASE + HH
  model.add(
    dome(M, { radius: 7.4, lift: 1.12, drumH: 3.6, drumR: 6.9, windows: 12, tag: 'main', y: domeY }),
  )
  ...
  for (let i = 0; i < 3; i++) {
    const s = add(
      new THREE.Mesh(new THREE.BoxGeometry(10 - i * 0.8, 0.42, 1.1), M.stoneDark),
      'step_' + i,
    )
