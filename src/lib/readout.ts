...
export interface Readout {
	prayer: string;
	ar: string;
	/** The selected city's name, or null when nothing is selected. */
	city: string | null;
	coord: string;
	clock: string;
...
export function pickCity(hover: { lat: number; lng: number } | null, centerLng: number, centerLat: number): City {
	let best = CITIES[0];
...
export function buildReadout({ city, nowMs, hover, centerLng, days, phases }: ReadoutInput): Readout {
	 ...
	 *
	 * Most of this function answers a question about somewhere — what time it is
	 * there, which prayer it is in, when its sun rises. With nothing selected there
	 * is no somewhere, so the frame falls back to 0°N 0°E, which is to say UTC.
	 * The parts that never needed a city — the sub-solar and sub-lunar points, the
	 * tally of every city by prayer, the pointer's own coordinates — are unaffected
	 * by that, and go on working exactly as before. The parts that did need one are
	 * blanked at the end rather than reported against a place nobody chose.
	 */
	const ref = city ?? { la: 0, lo: 0 };

	// Local solar model — always computed; it backs the globe and the fallback.
	const table = solarTable(ref.la, dec);
	const solarNow = (((utcH + ref.lo / 15 + eot / 60) % 24) + 24) % 24;
	const localPhase = phaseAt(ref.la, solarNow, dec, table);
	...
	const offsetHours = source === 'diyanet' && day ? day.offsetMin / 60 : ref.lo / 15;
	...
	const civilFromSolar = (solarHour: number) =>
		isNaN(solarHour) ? '—:—' : fmt(solarHour - eot / 60 - ref.lo / 15 + offsetHours);
	...
	if (hit) {
		...
		const bounds = ROWS.map(r => ({ label: r.label, solar: table[r.key] })).filter(b => !isNaN(b.solar));
		const next =
	...
	const arcMarks: ArcMark[] = ROWS.map(row => {
		...
		if (source === 'diyanet' && day) {
			...
			const v = table[row.key];
			hour = isNaN(v) ? NaN : v - eot / 60 - ref.lo / 15 + offsetHours;
		}
		return {
			label: row.label,
...
export function citiesInPhase(nowMs: number, phase: number, phases: PhaseTable | null) {
	...
	const centre = cities.length ? { lat: Math.atan2(z, Math.hypot(x, y)) / D, lon: Math.atan2(y, x) / D } : null;
