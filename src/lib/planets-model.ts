...
function bands(ctx: CanvasRenderingContext2D, count: number, colors: string[], seed: number, wobble = 14, alpha = 0.9) {
	const r = rnd(seed);
	for (let i = 0; i < count; i++) {
		...
		ctx.beginPath();
		ctx.moveTo(0, y);
		for (let x = 0; x <= TW; x += 32) ctx.lineTo(x, y + Math.sin((x / TW) * Math.PI * (2 + r() * 5) + i) * wobble);
...
function blobs(ctx: CanvasRenderingContext2D, n: number, seed: number, color: string, scale = 1) {
	const r = rnd(seed);
	ctx.fillStyle = color;
	for (let i = 0; i < n; i++) {
		const cx = r() * TW;
