 ...
 */
const BODY_FOR_PLANET: BodyId[] = ['mercury', 'venus', 'mars', 'jupiter', 'saturn', 'uranus', 'neptune'];
...
export function createCosmos(canvas: HTMLCanvasElement) {
	...
	return {
		...
		render({ map, nowMs, show, rimPx }: CosmosFrame): PlanetLabel[] {
			...
			const fade = zoom <= FADE_FROM ? 1 : zoom >= FADE_TO ? 0 : 1 - (zoom - FADE_FROM) / (FADE_TO - FADE_FROM);
